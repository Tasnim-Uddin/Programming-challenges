...
def LoadGreyScaleImage(FileIn, Grid, Header, Character):
    try:
        for Row in range(Header.Height):
            for Column in range(Header.Width):
...
def MirrorImage(Grid, Header):
    NewGrid = [[EMPTY_STRING for Column in range(MAX_WIDTH)] for Row in range(MAX_HEIGHT)]
    NewGrid = ClearGrid(NewGrid)
    for NewRow in range(MAX_HEIGHT):
        for NewColumn in range(MAX_WIDTH):
            NewGrid[NewRow][NewColumn] = Grid[NewRow][Header.Width - NewColumn - 1]
